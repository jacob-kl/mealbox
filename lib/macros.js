...
export const DIET_TYPES = {
  balanced: { label: 'Balanced', proteinPerKg: null, fatPercent: 0.28 },
  high_protein: { label: 'High protein', proteinPerKg: 2.4, fatPercent: 0.25 },
  low_fat: { label: 'Low fat', proteinPerKg: null, fatPercent: 0.18 },
  low_carb: { label: 'Low carb', proteinPerKg: null, fatPercent: 0.45 },
  high_carb: { label: 'High carb', proteinPerKg: 1.6, fatPercent: 0.2 },
};
...
export function feetInchesFromCm(cm) {
 ...
 */
export function calculateTargets({ sex, age, heightCm, weightLb, activityLevel, goal, dietType = 'balanced' }) {
  ...
  const preset = DIET_TYPES[dietType] || DIET_TYPES.balanced;
  const proteinPerKg = preset.proteinPerKg ?? (GOAL_PROTEIN_PER_KG[goal] ?? 1.8);
  const fatPercentOfCalories = preset.fatPercent ?? FAT_PERCENT_OF_CALORIES;

  const proteinG = proteinPerKg * weightKg;
  const fatCalories = calories * fatPercentOfCalories;
