...
export function classifyIngredientRole(ingredient) {
  ...
  if (proteinPer100Cal >= 6) return 'protein';
 ...
 * @param {Record<string, Object>} ingredientsByName
 * @param {Object} targets - {targetCalories, targetProtein}
 ...
 */
export function personalizeRecipe(recipe, ingredientsByName, { targetCalories, targetProtein }) {
  const baseServings = recipe.base_servings || 1;

  const lines = recipe.ingredients.map((line) => {
    const ingredient = ingredientsByName[line.ingredient];
    const role = classifyIngredientRole(ingredient);
    ...
    return { line, role, perServingQty, perServingMacros };
  });
  ...
  // Whatever calorie budget is left after protein + the fixed aromatics/
  // veg/fat lines is what sizes the carb/starch line(s) — this is the step
  // that actually keeps carbs in check instead of just multiplying with
  // everything else.
  let carbScale = 1;
  if (baseCarbCal > 0 && targetCalories != null) {
