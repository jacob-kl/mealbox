...
// Units attached directly to the number with no space, since that's how
// they're conventionally written ("200g", not "200 g").
const NO_SPACE_UNITS = new Set(['g', 'g_dry', 'g_raw', 'kg', 'ml', 'l']);

// Cooking measures read far more naturally as fractions than decimals -
// "1/4 tbsp" instead of "0.17tbsp". Eighths get a tighter tolerance since
// they're precise-but-uncommon in practice; the fractions people actually
// reach for (quarters, thirds, halves) get a wider net so odd decimals
// snap to the nearest sensible measurement rather than falling through to
// a raw number. Candidates are tried tightest-first and the CLOSEST match
// wins, so an exact 1/3 doesn't get swallowed by 1/4's wider net.
const FRACTION_UNITS = new Set(['tsp', 'tbsp', 'cup']);
// Bare countable units ("0.5 egg", "0.5 clove") have no unit suffix but
// still read better as "1/2" than "0.5" - same fraction treatment, no unit word.
const COUNTABLE_FRACTION_UNITS = new Set(['egg', 'clove', 'medium', 'avocado', 'pita']);
const FRACTION_CANDIDATES = [
  [1 / 8, '1/8', 0.03], [3 / 8, '3/8', 0.03], [5 / 8, '5/8', 0.03], [7 / 8, '7/8', 0.03],
  [1 / 4, '1/4', 0.09], [1 / 3, '1/3', 0.09], [1 / 2, '1/2', 0.09], [2 / 3, '2/3', 0.09], [3 / 4, '3/4', 0.09],
];

function toFractionDisplay(qty) {
  const whole = Math.floor(qty);
  const frac = qty - whole;
  if (frac < 0.05) return whole > 0 ? `${whole}` : '0';
  let best = null;
  for (const [value, label, tolerance] of FRACTION_CANDIDATES) {
    const diff = Math.abs(frac - value);
    if (diff < tolerance && (!best || diff < best.diff)) {
      best = { label, diff };
    }
  }
  if (best) return whole > 0 ? `${whole} ${best.label}` : best.label;
  // No clean fraction match within tolerance - fall back to a short decimal
  return String(Math.round(qty * 100) / 100);
}

// A small fraction of a cup or tbsp is more naturally expressed in the
// next smaller unit - nobody measures "1/16 cup", they measure 1 tbsp.
// Chains through cup -> tbsp -> tsp as needed so a tiny cup amount still
// lands somewhere sensible rather than stopping halfway.
function downgradeSmallUnit(qty, unit) {
  if (unit === 'cup' && qty < 0.2) {
    qty *= 16;
    unit = 'tbsp';
  }
  if (unit === 'tbsp' && qty < 0.34) {
    qty *= 3;
    unit = 'tsp';
  }
  return { qty, unit };
...
export function formatQty(qty, unit) {
  if (FRACTION_UNITS.has(unit)) {
    const downgraded = downgradeSmallUnit(qty, unit);
    const displayUnit = DISPLAYABLE_UNITS[downgraded.unit];
    const separator = NO_SPACE_UNITS.has(downgraded.unit) ? '' : ' ';
    return `${toFractionDisplay(downgraded.qty)}${separator}${displayUnit}`;
  }
  if (COUNTABLE_FRACTION_UNITS.has(unit)) {
    return toFractionDisplay(qty);
  }
  const displayUnit = DISPLAYABLE_UNITS[unit];
  if (!displayUnit) return `${qty}`;
  const separator = NO_SPACE_UNITS.has(unit) ? '' : ' ';
  return `${Math.round(qty * 100) / 100}${separator}${displayUnit}`;
}
