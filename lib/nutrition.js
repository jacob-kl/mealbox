...
function toFractionDisplay(qty) {
  ...
  if (frac < 0.02) return whole > 0 ? `${whole}` : '0';
...
export function formatQty(qty, unit) {
  ...
  const displayQty = FRACTION_UNITS.has(unit) ? toFractionDisplay(qty) : Math.round(qty * 100) / 100;
  const separator = NO_SPACE_UNITS.has(unit) ? '' : ' ';
  return `${displayQty}${separator}${displayUnit}`;
}
