...
export function generateWeek({
  ...
  // Per-member lunch schedule (which days, batch vs fresh) - falls back to
  // the household-wide mealDays/lunchPlan for any member without their own
  // entry, so existing single-schedule households keep working unchanged.
  ...
  // Each member's own effective dayConfig for THEIR individual meals (lunch,
  // snacks) - same shared breakfast/dinner/dessert flags as the household,
  // but lunch reflects this specific member's own schedule.
  function dayConfigFor(member, dayIndex) {
    return { ...(mealDays[dayIndex] || {}), lunch: lunchEnabledFor(member, dayIndex) };
  }
  ...
  //
  // The shared-meal share calculation needs *some* answer for "is lunch
  // active today" to renormalize against, even though lunch itself is now
  // decided per-member - "active if any member has lunch on" is a simple,
  // reasonable stand-in that doesn't require portioning shared meals
  // per-member (they're shared; splitting a single dinner recipe into
  // different-shaped days per person isn't something this models).
  for (let dayIndex = 0; dayIndex < 7; dayIndex++) {
    const anyLunchToday = members.some((m) => lunchEnabledFor(m, dayIndex));
    const dayConfig = { ...(mealDays[dayIndex] || {}), lunch: anyLunchToday };
  ...
  for (const member of members) {
    for (let dayIndex = 0; dayIndex < 7; dayIndex++) {
      ...
      const shares = computeMealShares(dayConfigFor(member, dayIndex), snacksPerDay);
  ...
  // member's snack share reflects their own lunch schedule for that day.
  if (snacksPerDay > 0) {
    for (let dayIndex = 0; dayIndex < 7; dayIndex++) {
      for (const member of members) {
        const shares = computeMealShares(dayConfigFor(member, dayIndex), snacksPerDay);
