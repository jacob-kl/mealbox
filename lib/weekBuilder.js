 ...
 */
function personalizeForHousehold(recipe, ingredientsByName, members, calorieShare) {
  if (!ingredientsByName || !recipe) return null;
  const avgCalorieTarget = (members.reduce((sum, m) => sum + m.targetCalories, 0) / members.length) * calorieShare;
...
function scalePortionsFor(macrosPerServing, calorieShare, members) {
  return members.map((m) => ({
    profileId: m.id,
    servings: servingsForTarget(macrosPerServing, m.targetCalories * calorieShare),
  }));
}

function sharedMealRow({ recipePool, dayIndex, mealType, filterCourse, filterCourseIn, storeCourse, calorieShare, proteinDensityTarget, members, blockedTags, cuisine, excludeIds, ingredientsByName }) {
  ...
  const personalized = personalizeForHousehold(recipe, ingredientsByName, members, calorieShare);
  const effectiveMacros = personalized?.macros ?? recipe.macros_per_serving;
  const portions = scalePortionsFor(effectiveMacros, calorieShare, members);
 ...
 */
function sharedMeals({ recipePool, dayIndex, mealType, calorieShare, proteinDensityTarget, members, blockedTags, cuisine, excludeIds, ingredientsByName }) {
  if (mealType === 'dinner') {
...
function individualMeal({ recipePool, dayIndex, mealSlot, member, mealType, blockedTags, cuisine, excludeIds, calorieShare, proteinDensityTarget, ingredientsByName }) {
  ...
  const servings = personalized ? 1 : servingsForTarget(recipe.macros_per_serving, member.targetCalories * calorieShare);
  ...
  for (let dayIndex = 0; dayIndex < 7; dayIndex++) {
    ...
    for (const member of members) {
      if (strategy === 'batch') {
        ...
        const servings = personalized ? 1 : servingsForTarget(recipe.macros_per_serving, member.targetCalories * shares.lunch);
        meals.push({
          dayIndex,
          mealSlot: 'lunch',
          profileId: member.id,
